...
export default function StrategicRiskMatrixPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex-1">
        <Header title="Strategic Risk Matrix" subtitle="Criticidad de negocio vs complejidad arquitectonica para priorizar inversiones" />
        <div className="p-6">
  ...
  const kpiCards = [
    { label: 'Zona Critica', value: q?.critical ?? 0, icon: ShieldAlert, color: 'text-danger' },
    ...
    <div className="flex-1">
      ...
      <div className="p-6 space-y-6">
        ...
        <ChartContainer
          ...
          subtitle="Tamano de burbuja = # interfaces | Color = proveedor"
          height="h-[500px]"
        >
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 20 }}>
              ...
              <YAxis
                ...
                domain={[0, 5]}
                ticks={[1, 2, 3, 4]}
                tickFormatter={(v: number) => ['', 'Baja', 'Media', 'Alta', 'Critica'][v] || ''}
                ...
                content={({ payload }) => {
                  ...
                  return (
                    <div className="bg-card border border-border rounded p-3 text-xs shadow-lg">
                      ...
                      <p>Techs unicas: <span className="font-medium">{d.uniqueTechs}</span></p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-lg p-3">
            ...
            <div><span className="font-semibold">Atencion Urgente</span><br />Alta criticidad + Alta complejidad</div>
          </div>
          ...
          <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/30 rounded-lg p-3">
            ...
            <div><span className="font-semibold">Operacion Normal</span><br />Baja criticidad + Baja complejidad</div>
          </div>
        </div>
        ...
        {data?.redZone?.length > 0 && (
          <div className="bg-card rounded-lg border border-border p-4">
            <h3 className="font-semibold mb-1">Top {data.redZone.length} Aplicaciones en Zona Critica</h3>
            <p className="text-xs text-muted-foreground mb-4">Apps con alta criticidad y alta complejidad — requieren atencion prioritaria</p>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-card">
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left p-2">Accion Sugerida</th>
                  </tr>
                </thead>
                <tbody>
